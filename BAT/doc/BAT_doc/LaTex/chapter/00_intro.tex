...
\begin{subequations}
  ...
  \end{align}
\end{subequations}
where $\Delta F_{Vth}$ is thermal preload change (see \equ{equ:dFVth}) and 
$F_Z$ is the preload loss due to embedding (see equXX).
...
\begin{subequations}
  ...
  \begin{align}
    ...
    &= \frac{\Delta L c_b c_c}{c_b + c_c} = \frac{\Delta L}{\delta_b + \delta_c}
    \label{equ:dFVth}
  \end{align}
\end{subequations}
...
\subsection{Stress in bolt at service}
to be filled